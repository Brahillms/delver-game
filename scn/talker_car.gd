...
func _physics_process(_delta: float) -> void:
	if Input.is_action_just_pressed("ui_accept") and is_within_talking_range and not shut_up:
		shut_up = true
		await %TextBoxManager.display_textbox(TextBoxManager.Init.OPENING, "player", "talking", "My car.")
		await %TextBoxManager.display_textbox(TextBoxManager.Init.IDLE, "player", "talking_looking_right", "It's seen quite a few dents.")
		await %TextBoxManager.display_textbox(TextBoxManager.Init.CLOSING, "player", "talking", "I love it all the same.")
